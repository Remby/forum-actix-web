...
async fn get_posts(
...
)->Result<HttpResponse,Error>{ let limit = query.limit.unwrap_or(20);
    let offset = query.offset.unwrap_or(0);

    let rows = sqlx::query!(
        "SELECT posts.id, posts.user_id, users.nickname,users.avatar, posts.title, posts.content, posts.images FROM posts JOIN users ON posts.user_id=users.id ORDER BY posts.created_at DESC LIMIT $1 OFFSET $2",
        limit,
        offset
    )
    ...
    let posts:Vec<PostResponse> = rows.into_iter().map(
        |row|PostResponse{
            id:row.id,
            user_id: row.user_id,
